  ...
  return (
    <div>
      <div className="page-header">
        ...
        <TrendingUp size={18} color="#c9a44a" />
        <span style={{ fontWeight: 700, color: '#fff', flex: 1 }}>Dashboard de Exames</span>
      </div>

      <div className="page-content">
        {loading ? (
          <p style={{ color: '#a0a0a0' }}>Carregando exames...</p>
        ) : !data || data.availableMarkers.length === 0 ? (
          <div className="card" style={{ textAlign: 'center', padding: 48 }}>
            <FlaskConical size={40} color="#c9a44a" style={{ margin: '0 auto 16px' }} />
            <p style={{ color: '#a0a0a0' }}>Nenhum exame cadastrado ainda.</p>
          </div>
        ) : (
          <>
            {/* Marker filter */}
            <div className="card" style={{ marginBottom: 20 }}>
              <div style={{ fontSize: 12, color: '#c9a44a', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 12 }}>
            ...
            {activeTimeline.length > 0 && (
              <div className="card" style={{ marginBottom: 20 }}>
                <h3 style={{ color: '#c9a44a', fontSize: 14, textTransform: 'uppercase', marginBottom: 20 }}>
                  Linha do Tempo
                </h3>
            ...
            {/* Summary table */}
            <div className="card">
              <h3 style={{ color: '#c9a44a', fontSize: 14, textTransform: 'uppercase', marginBottom: 16 }}>
                  ...
                  <thead>
                    <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                      {['Marcador', 'Último Valor', 'Unidade', 'Ref. Lab', 'Ref. Ótima', 'Status'].map(h => (
                        <th key={h} style={{ padding: '8px 12px', textAlign: 'left', color: '#a0a0a0', fontWeight: 600 }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {activeTimeline.map(marker => {
                      ...
                      const statusClass = { normal: 'badge-normal', baixo: 'badge-baixo', alto: 'badge-alto', subotimo: 'badge-subotimo' }[last?.status] || 'badge-normal';
                      return (
                        <tr key={marker.name} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                          <td style={{ padding: '10px 12px', color: '#fff', fontWeight: 500 }}>{marker.name}</td>
                          <td style={{ padding: '10px 12px', color: '#c9a44a', fontWeight: 700 }}>{last?.value ?? '—'}</td>
                          <td style={{ padding: '10px 12px', color: '#a0a0a0' }}>{marker.unit}</td>
                          <td style={{ padding: '10px 12px', color: '#a0a0a0' }}>
                            {marker.ref_min && marker.ref_max ? `${marker.ref_min} – ${marker.ref_max}` : '—'}
                          </td>
                          <td style={{ padding: '10px 12px', color: '#a0a0a0' }}>
                            {marker.optimal_min && marker.optimal_max ? `${marker.optimal_min} – ${marker.optimal_max}` : '—'}
                          </td>
